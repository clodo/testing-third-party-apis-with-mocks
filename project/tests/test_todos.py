...
class TestTodos(object):
    ...
    def test_getting_todos_when_response_is_ok(self):
        ...
        # Configure the mock to return a response with an OK status code. Also,
        # the mock should have a `json()` method that returns a list of todos.
    ...
    def test_getting_todos_when_response_is_not_ok(self):
        # Configure the mock to not return reponse with an OK status code.
        self.mock_get.return_value.ok = False

        # Call the service, shich will send a request to the server.
        response = get_todos()
